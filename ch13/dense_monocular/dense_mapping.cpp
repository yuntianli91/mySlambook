/*
 * @Description: this code is based on SLAM13 by GaoXiang
 * @Author: Yuntian Li
 * @Github: https://github.com/yuntinali91
 * @Date: 2019-07-22 15:05:03
 * @LastEditors: Yuntian Li
 * @LastEditTime: 2019-07-22 15:33:53
 */
#include <iostream>
#include <vector>
#include <fstream>
...
const double fx = 481.2f;	// camera intrinsic
const double fy = -480.0f;
const double cx = 319.5f;
const double cy = 239.5f;
const int ncc_window_size = 2;	// half window of ncc
const int ncc_area = (2*ncc_window_size+1)*(2*ncc_window_size+1); // pixel number of ncc area
const double min_cov = 0.1;	// min covariance for depth fusion
const double max_cov = 10;	// max covariance for depth fusion
...
// update depth and depth covariances
bool update( 
    const Mat& ref, 
    const Mat& curr, 
    const SE3d& T_C_R, 
...
// search corresponding pixel on epipolar
bool epipolarSearch( 
    const Mat& ref, 
    const Mat& curr, 
    const SE3d& T_C_R, 
...
// update depth and depth covariance by depth filter 
...
// transform from pixel to camera 
...
// transform from camera to pixel 
...
// check whether a pixel is inside 
inline bool inside( const Vector2d& pt ) {
    return pt(0,0) >= boarder && pt(1,0)>=boarder 
        && pt(0,0)+boarder<width && pt(1,0)+boarder<=height;
}

// show epipolar match result
...
// check input arguments
    ...
    // reading image filename and pose from dataset txt
    vector<string> color_image_files; 
    ...
    double init_depth   = 3.0;    // initial depth value for all pixels
    double init_cov2    = 3.0;    // initial depth covariances for all pixels
    
    Mat depth( height, width, CV_64F, init_depth );             // initial depth image
    Mat depth_cov( height, width, CV_64F, init_cov2 );          // initial depth covariance
    
    // update depth estimation by reading images
...
)
{
    ...
    while ( !fin.eof() )
    {
		// data format: tx, ty, tz, qx, qy, qz, qw ，TWC not TCW
        string image; 
        fin>>image; 
        double data[7];
...
#pragma omp parallel for
        ...
        {
            // jump pixel which is converged or diverged
			if ( depth_cov.ptr<double>(y)[x] < min_cov || depth_cov.ptr<double>(y)[x] > max_cov ) 
                continue;
            // search corresponding pixel on epipolar line 
            Vector2d pt_curr; 
            ...
            if ( ret == false ) // jump pixel failed matching
                continue; 
